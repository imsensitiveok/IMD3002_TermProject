...
def shelfBlock():
    ...
    blockMat = cmds.shadingNode('lambert', asShader=True, name=block.namespace + ':blockMat')
...
def sixBumpBlock():
    ...
    vertical = cmds.polyCube(w=block.sizeX, d=block.sizeZ, h=block.sizeY * 2,
                             name=block.namespace + ':vertical')[0]
    ...
    for i in range(2):
        for j in range(2):
            xBump(block, j, i, 1)  # Adding 4 bumps on the fron
            cmds.move(-0.4, x=True, relative=True)
...
def fourBumpBlock():
    ...
    mainBlock = cmds.polyCube(w=block.sizeX / 1.5, h=block.sizeY * 2, d=block.sizeZ * 1.5, 
                             name=block.namespace + ':mainBlock')[0]
    ...
    cmds.select(clear=True)
    cmds.select([mainCube] + cmds.ls(block.namespace + ':*', type='transform'))
    unifiedBlock = cmds.polyUnite(n=block.namespace + ':fourBumpBlock')[0]
    cmds.delete(ch=True) 
